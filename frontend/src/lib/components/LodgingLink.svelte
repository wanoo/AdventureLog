<script lang="ts">
	import type { Lodging, User } from '$lib/types';
	...
	$: {
		...
		if (searchQuery !== '') {
			filtered = filtered.filter((l) => {
				...
				const locationMatch = l.location?.toLowerCase().includes(searchQuery.toLowerCase()) || false;
		...
		if (filterOption === 'public') {
			...
			filtered = filtered.filter((l) => !l.collection);
		}

		filteredLodgings = filtered;
	}

	// Statistics
	$: totalLodgings = lodgings.length;
	$: unlinkedLodgings = lodgings.filter((l) => !l.collection).length;

	onMount(async () => {
		modal = document.getElementById('lodging_link_modal') as HTMLDialogElement;
		if (modal) {
			modal.showModal();
		}
		...
		try {
			...
			if (!res.ok) {
				console.error('Failed to fetch lodgings:', res.status, res.statusText);
				isLoading = false;
				return;
			}

			const allLodgings = (await res.json()) as Lodging[];
			// Filter out lodgings that are already linked to this collection
			lodgings = allLodgings.filter(
				(l) => !l.collection || (typeof l.collection === 'string' ? l.collection : l.collection?.id) !== collectionId
			);
	...
	async function linkLodging(lodging: Lodging) {
		...
		// Check if lodging is already in another collection
		const existingCollection = lodging.collection;
		...
		try {
			const response = await fetch(`/api/lodging/${lodging.id}/`, {
				method: 'PATCH',
				headers: {
			...
			if (response.ok) {
				...
				// Remove from list
				lodgings = lodgings.filter((l) => l.id !== lodging.id);
				dispatch('add', updated);
			} else {
				console.error('Failed to link lodging:', await response.text());
			}
		} catch (error) {
			console.error('Error linking lodging:', error);
		} finally {
			linkingId = null;
		}
	}
...
<dialog id="lodging_link_modal" class="modal backdrop-blur-sm">
		...
		>
				...
				<div class="flex items-center gap-3">
					...
					<div>
						<h1 class="text-3xl font-bold text-primary bg-clip-text">
							{$t('adventures.link_lodging')}
						</h1>
						<p class="text-sm text-base-content/60">
							{filteredLodgings.length}
							{$t('worldtravel.of')}
							{totalLodgings}
							{$t('adventures.lodging').toLowerCase()}
						</p>
					</div>
				</div>
				...
				<div class="hidden md:flex items-center gap-2">
					<div class="stats stats-horizontal bg-base-200/50 border border-base-300/50">
						<div class="stat py-2 px-4">
							...
							<div class="stat-value text-lg text-info">{totalLodgings}</div>
						</div>
						<div class="stat py-2 px-4">
							<div class="stat-title text-xs">{$t('adventures.unlinked')}</div>
							<div class="stat-value text-lg text-success">{unlinkedLodgings}</div>
						</div>
					</div>
				</div>
			...
			<div class="mt-4 flex items-center gap-4">
				<div class="relative flex-1 max-w-md">
					...
					<input
						type="text"
						placeholder="{$t('navbar.search')} {$t('adventures.name')}, {$t('adventures.location')}..."
			...
			<div class="px-2">
				{#if filteredLodgings.length === 0}
					<div class="flex flex-col items-center justify-center py-16">
						...
						{:else}
							<h3 class="text-xl font-semibold text-base-content/70 mb-2">
								{$t('adventures.no_lodging_available')}
							</h3>
							<p class="text-base-content/50 text-center max-w-md">
								{$t('adventures.all_lodging_linked')}
							</p>
						{/if}
					</div>
				{:else}
					...
					<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-6 p-4">
						{#each filteredLodgings as lodging (lodging.id)}
							<div class="relative">
								<LodgingCard {lodging} user={user} />
								<div class="absolute bottom-4 right-4">
									<button
										class="btn btn-primary btn-sm gap-2"
										disabled={linkingId === lodging.id}
										on:click={() => linkLodging(lodging)}
									>
										{#if linkingId === lodging.id}
