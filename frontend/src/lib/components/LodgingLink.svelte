<script lang="ts">
	...
	$: {
		...
		if (filterOption === 'public') {
			...
			filtered = filtered.filter((l) => !l.collections || l.collections.length === 0);
		}
	...
	$: unlinkedLodgings = lodgings.filter((l) => !l.collections || l.collections.length === 0).length;
	...
	onMount(async () => {
		...
		try {
			...
			lodgings = allLodgings.filter(
				(l) => !l.collections || !l.collections.includes(collectionId)
			);
