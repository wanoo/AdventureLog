<script lang="ts">
	...
	$: {
		...
		if (filterOption === 'public') {
			...
			filtered = filtered.filter((t) => !t.collection);
		}
	...
	$: unlinkedTransportations = transportations.filter((t) => !t.collection).length;
	...
	onMount(async () => {
		...
		try {
			...
			transportations = allTransportations.filter(
				(t) => !t.collection || (typeof t.collection === 'string' ? t.collection : t.collection?.id) !== collectionId
			);
	...
	async function linkTransportation(transportation: Transportation) {
		...
		// Check if transportation is already in another collection
		const existingCollection = transportation.collection;
