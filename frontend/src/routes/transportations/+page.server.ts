...
export const load = (async (event) => {
	if (!event.locals.user) {
		...
		let initialFetch = await event.fetch(
			`${serverEndpoint}/api/transportations/filtered?types=${typeString}&order_by=${order_by}&order_direction=${order_direction}&page=${page}&is_visited=${is_visited}&is_public=${is_public}&ownership=${ownership}&min_rating=${min_rating}`,
			{
				headers: {
