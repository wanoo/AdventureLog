...
class TransportationViewSet(FilteringMixin, ViewsetUtilsMixin, viewsets.ModelViewSet):
    ...
    def additional_info(self, request, pk=None):
        """Get transportation with additional sunrise/sunset information."""
        transportation = self.get_object()

        # Get base transportation data
        serializer = self.get_serializer(transportation)
        response_data = serializer.data

        # Add sunrise/sunset data (using origin location)
        response_data['sun_times'] = self._get_sun_times(transportation, response_data.get('visits', []))

        return Response(response_data)

    def _get_sun_times(self, transportation, visits):
        """Get sunrise/sunset times for transportation visits (using origin location)."""
        sun_times = []

        for visit in visits:
            date = visit.get('start_date')
            # Use origin coordinates for transportation
            if not (date and transportation.origin_longitude and transportation.origin_latitude):
                continue

            api_url = (
                f'https://api.sunrisesunset.io/json?'
                f'lat={transportation.origin_latitude}&lng={transportation.origin_longitude}&date={date}'
            )
    ...
    def filtered(self, request):
        """Filter transportations by type, visit status, and visibility."""
        from django.conf import settings
        ...
        if 'all' in types or not types:
            queryset = Transportation.objects.filter(base_filter).distinct()
        else:
    ...
    def history(self, request, pk=None):
        """Get audit history for a transportation and its related content (collaborative mode only)."""
        ...
        # Get content types for transportation, images, and attachments
        ...
        # Get IDs of images and attachments belonging to this transportation (including soft-deleted)
        image_ids = list(transportation.images.all().values_list('id', flat=True))
        attachment_ids = list(transportation.attachments.all().values_list('id', flat=True))

        # Build query for all related logs
        logs_query = Q(content_type=transportation_ct, object_id=transportation.pk)
    ...
    def revert(self, request, pk=None, log_id=None):
        ...
        # Verify the log entry belongs to this transportation or its related content
        transportation_ct = ContentType.objects.get_for_model(Transportation)
        image_ct = ContentType.objects.get_for_model(ContentImage)
        attachment_ct = ContentType.objects.get_for_model(ContentAttachment)

        is_transportation_log = log_entry.content_type == transportation_ct and str(log_entry.object_id) == str(transportation.pk)
        is_image_log = log_entry.content_type == image_ct and transportation.images.filter(id=log_entry.object_id).exists()
        is_attachment_log = log_entry.content_type == attachment_ct and transportation.attachments.filter(id=log_entry.object_id).exists()

        if not (is_transportation_log or is_image_log or is_attachment_log):
            return Response({"error": "This audit log entry does not belong to this transportation"}, status=403)

        # Check permission: only the user who made the change, the transportation owner, or an admin can revert
        can_revert = (
            log_entry.user == request.user or
            transportation.user == request.user or
